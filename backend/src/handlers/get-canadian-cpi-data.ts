  ...
  seriesIds: z
    ...
    .refine(
      (keys) => {
        ...
        return keys.every((key) => key in CpiSeriesIdCanada);
      },
      {
        error: (issue) => {
          const keys = issue.input as string[];
          const invalidKey = keys.find((key) => !(key in CpiSeriesIdCanada));
          return `Invalid series ID: ${invalidKey}. Must be one of: ${Object.keys(CpiSeriesIdCanada).join(', ')}`;
        },
      },
    )
