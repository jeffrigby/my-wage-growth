...
describe('get-us-cpi-data integration tests', () => {
  ...
  describe('successful execution', () => {
    it('should process single series with default parameters', async () => {
      ...
      expect(s3Call.args[0].input.Key).toBe('data/CPI_U_ALL.json');
      expect(s3Call.args[0].input.Body).toContain('2023-01');
    });
    ...
    it('should handle empty CPI data from BLS API', async () => {
      ...
      const s3Call = s3Mock.commandCalls(PutObjectCommand)[0];
      expect(s3Call.args[0].input.Body).toContain('"months":{}');
    });
  });
  ...
  describe('data transformation', () => {
    it('should correctly group data by series ID', async () => {
      ...
      const cpiAllCall = s3Calls.find((call) => call.args[0].input.Key === 'data/CPI_U_ALL.json');
      ...
      const cpiFoodCall = s3Calls.find((call) => call.args[0].input.Key === 'data/CPI_U_FOOD.json');
    ...
    it('should handle data with footnotes', async () => {
      ...
      expect(result.status).toBe('success');
      const s3Call = s3Mock.commandCalls(PutObjectCommand)[0];
      const uploadedData = JSON.parse(s3Call.args[0].input.Body as string);
